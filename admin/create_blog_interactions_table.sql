-- Create blog_interactions table to track likes and views
...
CREATE INDEX IF NOT EXISTS idx_blog_interactions_created_at ON public.blog_interactions(created_at);
...
CREATE OR REPLACE FUNCTION public.add_post_like(post_uuid UUID, user_id TEXT)
RETURNS BOOLEAN AS $$
BEGIN
    INSERT INTO public.blog_interactions (post_id, user_identifier, interaction_type)
    VALUES (post_uuid, user_id, 'like')
    ON CONFLICT (post_id, user_identifier, interaction_type) DO NOTHING;
...
CREATE OR REPLACE FUNCTION public.remove_post_like(post_uuid UUID, user_id TEXT)
RETURNS BOOLEAN AS $$
BEGIN
    DELETE FROM public.blog_interactions
    WHERE post_id = post_uuid 
    AND user_identifier = user_id 
    AND interaction_type = 'like';
...
-- Function to add view to a post
CREATE OR REPLACE FUNCTION public.add_post_view(post_uuid UUID, user_id TEXT)
RETURNS BOOLEAN AS $$
BEGIN
    INSERT INTO public.blog_interactions (post_id, user_identifier, interaction_type)
    VALUES (post_uuid, user_id, 'view')
    ON CONFLICT (post_id, user_identifier, interaction_type) DO NOTHING;
