...
TO authenticated
USING (EXISTS (SELECT 1 FROM public.admin_users WHERE public.admin_users.email = auth.email()))
WITH CHECK (EXISTS (SELECT 1 FROM public.admin_users WHERE public.admin_users.email = auth.email()));
...
DROP POLICY IF EXISTS "Allow admin full access to blog categories" ON public.blog_categories;
CREATE POLICY "Allow admin full access to blog categories"
