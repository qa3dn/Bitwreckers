...
export function StudentsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const p = await ensurePage('students', 'Students')
        setPage(p)
  ...
  return (
    <Layout>
      <div className="card">
        <h1 style={{ marginTop: 0 }}>Students Page</h1>
        <button className="btn" onClick={async () => {
          const created = await apiFetch<Section>(`/admin/sections`, { method: 'POST', body: { page_id: page.id, key: 'students-hero', order_index: sections.length }, auth: true })
