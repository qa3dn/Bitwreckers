...
export function SolutionsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const p = await ensurePage('solutions', 'Solutions')
        setPage(p)
  ...
  return (
    <Layout>
      <div className="card">
        <h1 style={{ marginTop: 0 }}>Solutions Page</h1>
        <button className="btn" onClick={async () => {
          const created = await apiFetch<Section>(`/admin/sections`, { method: 'POST', body: { page_id: page.id, key: 'solutions-hero', order_index: sections.length }, auth: true })
