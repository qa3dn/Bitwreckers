...
export function ServicesPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const p = await ensurePage('services', 'Services')
        setPage(p)
  ...
  return (
    <Layout>
      <div className="card">
        <h1 style={{ marginTop: 0 }}>Services Page</h1>
        <button className="btn" onClick={async () => {
          const created = await apiFetch<Section>(`/admin/sections`, { method: 'POST', body: { page_id: page.id, key: 'services-hero', order_index: sections.length }, auth: true })
